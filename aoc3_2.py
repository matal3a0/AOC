...
while True:
    ...
    # Value to insert
    a = memory[posy][posx+1] + memory[posy][posx-1] + memory[posy+1][posx] + memory[posy-1][posx] + memory[posy+1][posx+1] + memory[posy+1][posx-1] + memory[posy-1][posx-1] + memory[posy-1][posx+1]
    memory[posy][posx] = a
    # If larger than address, we have the answer
    if (a > address):
        print "Answer: ", a
        break
